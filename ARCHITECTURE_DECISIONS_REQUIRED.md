...
| D-13 | Decimal precision standard | Money mostly 2dp (10/2, 14/2); qty 3dp (8/3–14/3); `Product.cost` 2dp | (a) money 2dp / qty 3dp everywhere; (b) **money 2dp, qty 3dp, internal unit-cost 4dp** | **(b)** — 4dp avg-cost avoids drift on small base units (gram-level recipes) while presenting 2dp. Trade-off: one more internal column convention; `Product.cost` (2dp) stays a display mirror. | EN+FA | 3, 6, 7 | **Selected: Option B — the unit-conversion half implemented as `Decimal(16,6)` conversion factors + 3dp HALF_UP base-quantity quantize (Sprint 2 Batch 1, `00fddae`, 2026-07-14; `pos/services/units.py`). The 4dp internal unit-cost half of D-13 (avg-cost precision) remains Open — it rides with D-07/D-35 at Gate G3, not G2. Gate G2 sign-off (unit/inventory precision only) recorded 2026-07-15 (§3.5, §4); ADR promotion still pending (R-M).** |
...
### 3.4a Sprint 2 catalog naming decisions (appended after D-35 — gate per §3.0, never renumbered)

| ID | Decision | Context | Options | Recommendation + trade-offs | Owner | Blocks | Status |
|---|---|---|---|---|---|---|---|
| D-36 | `Product.product_type` enum wording | Implemented (Sprint 2 Batch 3, 2026-07-15) as `stock_item, ingredient, prep_item, recipe_product, resale, packaging, service, bundle, fixed_asset` ([pos/services/product_types.py](superpos_backend/pos/services/product_types.py)); `MASTER_DATA_CONTRACT §11` and the original revised plan instead spell `non_stock`, `bundle_combo`, `fixed_asset_purchase_only` (no `resale`/`prep_item`/`packaging`) | (a) rename the enum to match `MASTER_DATA_CONTRACT §11`; (b) **keep the implemented wording, record the contract as superseded** | **(b)** — the implemented list is already shipped, tested (52 tests in `test_product_foundation.py`), and zero real tenant data is denominated in it yet (`0023_product_foundation` not `--apply`'d against production); a rename now is pure relabeling, a rename after real data exists would be a live-data migration. `service` continues to cover the contract's non-stock intent. | BO | none (Sprint 2 Batch 3/4, informs future ADR promotion) | **Selected: Option B (owner directive, 2026-07-15). Not gated — recorded for traceability only.** |
| D-37 | `Product.show_on_pos` vs `visible_in_pos` naming | Implemented as `show_on_pos` (Sprint 2 Batch 3, 2026-07-15); PRD v3.6 §26.1 names the same flag `visible_in_pos` | (a) rename to `visible_in_pos` to match PRD v3.6; (b) **keep `show_on_pos`, record PRD as superseded on this point** | **(b)** — already shipped and tested; a rename is a pure relabeling with no behavioral difference, not worth a migration + FE contract churn before Batch 5 consumes the field. | BO | none (Sprint 2 Batch 3/4) | **Selected: Option B (owner directive, 2026-07-15). Not gated — recorded for traceability only.** |
...
**2026-07-15 — Owner authorization to close G1/G2 (Sprint 2 Batch 4 planning session)**
- Participant: **Business Owner** (session authorization, in-conversation directive
  preceding Sprint 2 Batch 4 work; see `IMPLEMENTATION_PROGRESS.md` Batches 1–3 for
  the underlying execution record).
- Selections recorded:
  - **G1 (D-01): confirmed as implemented** — SalesCategory + InventoryCategory
    split (Sprint 2 Batch 2, 2026-07-14). No changes requested to the shipped
    design.
  - **G2 (D-13, units/inventory half only): confirmed as implemented** —
    `Decimal(16,6)` conversion factors + 3dp HALF_UP base-quantity quantize
    (Sprint 2 Batch 1, 2026-07-14). The avg-cost 4dp half of D-13 stays Open
    under G3 (tracks D-07/D-35, unrelated to units).
  - **D-36 (product_type enum wording): Option B** — keep the implemented
    wording (`resale`, `bundle`, `fixed_asset`, …); `MASTER_DATA_CONTRACT §11`
    recorded as superseded on this point.
  - **D-37 (`show_on_pos` naming): Option B** — keep `show_on_pos`; PRD v3.6
    §26.1's `visible_in_pos` recorded as superseded on this point.
- **Gate impact: G1 and G2 (units/inventory scope) sign-off is now recorded**
  in §4 below — the R-M sign-off half of each gate's exit criteria is
  satisfied. **ADR promotion (folding D-01/D-13 into ADR documents + the next
  PRD/FLOW/DOMAIN/DESIGN version, per R-M) has NOT happened yet** — that is a
  separate follow-up documentation task; until it lands, v3.6 stays
  authoritative per R-M and the gates are not yet fully closed in the strict
  R-M sense, only sign-off-complete. D-13's avg-cost precision half stays Open
