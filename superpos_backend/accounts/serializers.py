...
class BranchPaymentMethodSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        # Gate A: at most one ACTIVE default route per (branch, method_type).
        # `method_type` lives on the related PaymentMethod, so a DB
        # UniqueConstraint cannot express this — the serializer is the guard,
        # with `resolve_branch_payment_method`'s deterministic ordering
        # (`-is_default`, `-id`) as the backstop for pre-existing bad data
        # (which migration 0017 also demotes).
