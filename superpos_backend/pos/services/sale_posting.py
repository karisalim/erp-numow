...
Routing policy (Release Gate A — strict):
    * EVERY completed cash/card/wallet sale must resolve to an active
      ...
      accounts data migration `0017_backfill_branch_payment_routing` provisions
      default routing for every pre-existing branch, so an unrouted branch is
      always a configuration error, never an expected state.
...
def resolve_branch_payment_method(*, tenant, branch, method):
    ...
    Selection is deterministic: `-is_default` puts the default first and
    `-id` breaks any remaining tie by newest row, so even bad data (two
...
def post_sale_ledgers(*, sale, method, customer=None, actor_user=None):
    ...
    if bpm is None:
        # Gate A: no legacy skip. Every completed cash/card/wallet sale must
